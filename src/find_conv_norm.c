...
char	*ft_norm_u(char *res, t_s *s)
{
	if (s->f->point == TRUE)
		s->f->zero = FALSE;
	if (s->c->nb == 0 && s->f->point == TRUE && s->pres == 0) // nb?
		s->champ += 1;
	s->f->plus = FALSE;
	s->f->space = FALSE;
...
char	*ft_norm_x(char *res, t_s *s, int i)
{
	if (s->f->point == TRUE)
		s->f->zero = FALSE;
	if (s->c->unb == 0 && s->f->point == FALSE && s->f->zero == FALSE && s->f->moins == FALSE)
		s->champ -= 1;
	s->f->plus = FALSE;
	s->f->space = FALSE;
	if (s->str[i] == 'X')
...
char	*ft_norm_dio(char *res, t_s *s, int i)
{
	...
	if (s->c->nb == 0 && (s->str[i] == 'd' || s->str[i] == 'i') && s->pres == 0 && s->f->point == TRUE)
		s->champ += 1;
	if (s->c->unb == 0 && s->str[i] == 'o' && s->f->point == TRUE && s->pres == 0)
		s->champ += 1;
	...
	if (s->f->point == TRUE && s->str[i] == 'o')
		s->f->zero = FALSE;
	res = (s->str[i] == 'd' || s->str[i] == 'i') ? ft_pf_d(s, s->c->nb) : ft_pf_u(s, s->c->unb);
	if (s->f->hash == TRUE && ((s->f->point == TRUE && s->pres == 0 && s->c->unb == 0) || s->c->unb != 0))
		res = ft_hashzero(res);
	return (res);
}
